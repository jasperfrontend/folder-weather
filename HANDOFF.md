...
- `background.js` — scheduling + onboarding only: `onboard()` (ancestor-aware), `updateManaged()`, alarm, settings-change reschedule, and bookmark-event listeners for near-instant quick-add + eager delete cleanup. A `_busy` guard + onboard-returns-`[]`-for-our-own-rows prevent the render→event→render cascade.
...
2. **updateManaged()** — iterates every `fw_config_*` key, confirms the folder still exists (`bookmarks.get(id)`), and re-renders its rows. If the folder is gone, it removes the config, status, and tracked children.

**renderInto(folderId, config)** (in `engine.js`) geocodes the city (cached), fetches the forecast (unit-aware, coord-cached), splits `config.template || settings.defaultTemplate` on `|`, renders each segment, then **wipes only its own previously-created children** (tracked by id in `fw_children_<id>`) and recreates them as bookmark rows with `type: "separator"` dividers between. Rows link to a Google weather search for the city. Writes a `fw_status_<id>` record (ok / error) so the UI can surface failures.
...
**Cascade safety:** the extension's own row writes fire `bookmarks.onCreated/onRemoved`. A `_busy` flag plus the fact that `onboard()` returns `[]` for non-`[w...]` changes means rendering rows never retriggers more rendering.
